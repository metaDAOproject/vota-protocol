...
export function getVoteAccounts(config: web3.Keypair, program: Program<VoteMarket>, gaugeMeisterData, owner: web3.PublicKey) {
    ...
    let [escrow, escrowBump] = anchor.web3.PublicKey.findProgramAddressSync(
        [Buffer.from("Escrow"), gaugeMeisterData.locker.toBuffer(), owner.toBuffer()],
    ...
    let [epochGauge, epochGaugeBump] = anchor.web3.PublicKey.findProgramAddressSync(
        ...
        GAUGE_PROGRAM_ID);
    return {voteDelegate, escrow, gaugeVoter, gaugeVote, epochGaugeVoter, epochGaugeVote, epochGauge};
}