...
pub fn update_anchor_toml(table: &mut Value, update_info: Vec<AddressInfo>) {
    let addresses = table
        .get_next_table("test").unwrap()
        .get_next_table("validator").unwrap()
        ...
        Some(v) => {
            if let Value::Array(accounts) = v {
                ...
                for info in update_info {
                    ...
                    account.insert("address".to_string(), Value::String(info.pubkey.to_string()));
                    account.insert("filename".to_string(), Value::String(
                        format!("./test-accounts/{}.json", info.name)));
...
filename = "./test-accounts/epoch-gauge.json"
"#;
    let new_addresses = vec![AddressInfo { name: "epoch-gauge".to_string(), pubkey: pubkey!("7Jmy8EEofM24NRcy5BUmqHgYmXqo5P2N35RBQ6uuMatt")}];
